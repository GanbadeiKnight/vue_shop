<template>
	<div>
		...
		<el-card>
			<!-- 警告区域 -->
			<el-alert show-icon title="设置参数时，只允许为第三级分类设置相关参数！" type="warning">
			</el-alert>
			...
			<el-row class="cat_opt">
				<el-col>
					...
					<el-cascader v-model="selectedCateKeys" :options="cateList"
						:props="{ expandTrigger: 'hover',...cateProps,checkStrickly:'true'}" @change="handleChange"
	...
	export default {
		...
		methods: {
			...
			async getCateList() {
				...
				// console.log('来自getCateList的提示' + this.cateList)
			},
			// 一旦级联选择框发生变化便会触发此函数
			handleChange() {
				this.getParamsData()
			...
			// Tab页签点击事件的处理函数
			handleTabClick() {
				console.log('来自handleTabClick:' + this.activeName)
				this.getParamsData()
			},
			// 获取参数的列表数据
			async getParamsData() {
				...
				// 证明选中的是三级分类
				console.log('来自getParamsData的提示' + this.selectedCateKeys)
			...
			addParams() {
				this.$refs.addFormRef.validate(async valid => {
					...
					const {
						data: res
					} = await this.$http.post(`categories/${this.cateId}/
					attributes`, {
			...
			editParams() {
				this.$refs.editFormRef.validate(async valid => {
					...
					const {
						data: res
					} = await this.$http.put(`categories/${this.cateId}/attributes/
			...
			async removeParams(attr_id) {
				const confirmResult = await this.$confirm('将会永久性删除该参数,是否继续?', '提示', {
				...
				const {
					data: res
				} = await this.$http.delete(`categories/${this.cateId}/attributes/
					${attr_id}`)
